...
export default function App() {
  ...
  async function handleIdentityUnlocked(data: IdentityData) {
    ...
    // Open encrypted message store. Secrets (SEK, signing seed, peerId) are kept
    // in memory only and never written to sessionStorage — a refresh requires
    // re-unlocking. This denies any XSS a readable copy of the storage key (M2).
    try {
  ...
  onMount(() => {
    // No session secrets are cached, so a fresh load always shows the unlock
    // screen (handled by the Show fallback). The message history in IndexedDB
    // stays encrypted at rest until the identity is unlocked again.
  });
