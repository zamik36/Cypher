...
**Тесты:** 45 тестов, 0 failures. `cargo clippy --workspace` — 0 warnings.
...
Для мобильных: SQLCipher для хранения:
- Истории чатов (опционально, по желанию пользователя)
- Частично принятых файлов (для resume)
- Текущих рatchet состояний (для offline delivery)

Реализовать `crates/p2p-client-core/src/storage.rs` как полноценное
зашифрованное хранилище с `sqlcipher` + `rusqlite`.
...
Phase 11 (UX)           ⚠️ desktop UX готов; storage + voice не начаты
```
...
| Test coverage | 45 tests + 5 fuzz | >200 tests + fuzzing |
